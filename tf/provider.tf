terraform {
  # Commented out whilst using localstack
  # backend "s3" {}
  required_providers {
    aws = {
...
provider "aws" {
  # below only used for lcoalstack - in real life would use aws roles
  access_key = "test"
  secret_key = "test"
  region     = "us-east-1"
  default_tags {
    tags = {
      created-by = "path/to/repo" # Repo path - comes in handy for tracking down resource creation in large environments!
    }
  }
  # below is purely to fudge localstack endpoints
  endpoints {
