  ...
  statement {
    ...
    principals {
      ...
      identifiers = [
        ...
        var.pii == true ? "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/pii_s3_access" : "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/s3_access"
      ]
    }
  }

  dynamic "statement" {
    for_each = var.website == true ? [1] : []
    content {
      sid    = "cloudFront"
      effect = "Allow"
